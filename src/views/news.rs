...
pub fn build_news_view(app: &App, theme: &GlTheme, width: f32, height: f32) -> PanelBuilder {
    ...
    panel()
        ...
        .child(
            build_news_content(app, theme, metrics.inner_width, &spacing).flex_grow(1.0),
        )
...
fn build_news_content(
    ...
    panel()
        ...
        // Headlines panel (30%)
        .child(
            titled_panel("Headlines", theme, build_headlines_list(app, theme, width))
                .proportion(30.0) // Use proportion to account for gaps (official pattern)
                .flex_shrink(0.0),
        )
        // Content panel (70%)
        .child(
            build_content_panel(app, theme)
            .proportion(70.0) // Use proportion to account for gaps (official pattern)
            .flex_shrink(0.0),
        )
}

/// Build the headlines list (titles only, compact)
fn build_headlines_list(app: &App, theme: &GlTheme, width: f32) -> PanelBuilder {
    ...
    let mut container = panel()
        .flex_direction(FlexDirection::Column)
        .gap(2.0)
        .flex_grow(1.0); // Fill the available panel height
